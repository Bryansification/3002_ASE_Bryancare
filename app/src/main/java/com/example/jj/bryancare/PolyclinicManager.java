            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if(dataSnapshot.getValue() == null)
                    ...
                    if((int) patients < queuePosition) {
                        ...
                        boolean add = false;
                        accountManager.addToQueue(polyclinicName, queue, accountManager.getCurrentUser().getQueueNumber(), (int) patients, add);
    ...
    public void addToQueue(final String queue, final int queueNumber) {
        ...
        database.child("Polyclinics").child(getCurrentPolyclinic().getName()).child(queue).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                AccountManager accountManager = new AccountManager();
                boolean add = true;
                accountManager.addToQueue(getCurrentPolyclinic().getName(), queue, queueNumber, queuePosition, add);
